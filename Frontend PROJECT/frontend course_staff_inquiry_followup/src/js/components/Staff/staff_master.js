...
export default function Staff() {
    ...
    const addStaff = async (newStaff) => {
        const url = 'http://localhost:5210/api/pta/addstaff';
    ...
    const updateStaff = async (id, staff) => {
        const url = `http://localhost:5210/api/admin/updatestaff/${id}`;
       // const token = localStorage.getItem('jwt1');
        try {
            const response = await fetch(url, {
                method: 'PUT',
    ...
    const deleteStaff = async (staffId) => {
        const url = `http://localhost:5210/api/admin/deletestaff/${staffId}`;
        //const token = localStorage.getItem('jwt1');
        try {
            const response = await fetch(url, {
                method: 'DELETE',
