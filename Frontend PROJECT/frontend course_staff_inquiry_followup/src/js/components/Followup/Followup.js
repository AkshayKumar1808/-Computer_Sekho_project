...
function Followup() {
  ...
  const fetchFollowups = async () => {
    const url = 'http://localhost:5210/api/pta/followup';
    try {
  ...
  const updateFollowup = async (followup) => {
    const url = `http://localhost:5210/api/admin/updatefollowup/${followup.followupId}`;
...
function Followup() {
  ...
  const addfollowup = async (followup) => {
    const url = 'http://localhost:5120/api/admin/addfollowup';
    const token = localStorage.getItem('jwt1');
    try {
      const response = await fetch(url, {
        method: 'POST',
